"""Metrics workflow for polar-twist EP experiment."""
...
from mess.experiments.polar_twist_ep.benchmark_runtime import run as run_benchmark
from mess.experiments.polar_twist_ep.compute_metrics import run as run_metrics
from mess.experiments.polar_twist_ep.config import ExperimentConfig, build_context
from mess.experiments.polar_twist_ep.report_availability import run as run_availability
...
def run(config: Optional[ExperimentConfig] = None) -> None:
    ...
    registry.add(
        ...
        kind="table",
        description="Runtime summary from chain metadata.",
    )
    registry.add(
        path=Path(metrics_summary["report_path"]),
        kind="table",
        description="ESS/MSJD summary with EP replicate aggregation.",
    )
    registry.add(
        path=Path(metrics_summary["per_chain_path"]),
        kind="table",
        description="Per-chain ESS/MSJD metrics after burn-in.",
    )
    registry.add(
        path=Path(metrics_summary["per_replicate_path"]),
        kind="table",
        description="Per-replicate aggregated metrics for MESS and EP-ESS.",
    )
