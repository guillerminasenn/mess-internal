"""Notebook-parity checklist for phase-2 plotting artifacts."""
...
from mess.experiments.advection_diffusion_dim_sweep_shared_draws_pcn_mpcn.phase2_helpers import phase2_dirs
...
def run(config: Optional[ExperimentConfig] = None) -> Dict[str, object]:
    ...
    md_path = manifests_dir / "phase2_notebook_parity_checklist.md"
    json_path = manifests_dir / "phase2_notebook_parity_checklist.json"
    summary = write_checklist(
        checks=checks,
        markdown_path=md_path,
        json_path=json_path,
        title="Notebook 11 Phase-2 Parity Checklist",
    )

    print(f"Saved parity checklist markdown: {md_path}")
    print(f"Saved parity checklist JSON: {json_path}")
...
def main() -> None:
    summary = run()
    print("\nPhase-2 parity checklist summary:")
