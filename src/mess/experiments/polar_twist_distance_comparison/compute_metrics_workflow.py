"""Metrics workflow for polar-twist distance comparison."""
...
from mess.experiments.polar_twist_distance_comparison.benchmark_runtime import run as run_benchmark
from mess.experiments.polar_twist_distance_comparison.compute_metrics import run as run_metrics
from mess.experiments.polar_twist_distance_comparison.config import ExperimentConfig, build_context
from mess.experiments.polar_twist_distance_comparison.report_availability import run as run_availability
...
def run(config: Optional[ExperimentConfig] = None) -> None:
    ...
    registry.add(
        ...
        kind="table",
        description="ESS/MSJD summary for transition-matrix variants.",
    )
