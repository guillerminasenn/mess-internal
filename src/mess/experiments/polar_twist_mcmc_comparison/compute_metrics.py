...
def _mess_cost_metrics(payload, cfg: ExperimentConfig, m: Optional[int], ess_vec: np.ndarray) -> Dict[str, float]:
    unavailable = {
    ...
    if m is None or "mess_subiters_per_iter" not in payload:
        return unavailable
...
def run(config: Optional[ExperimentConfig] = None) -> Dict[str, Any]:
    ...
    for chain_file in sorted(outdir.glob("chain_*.npz")):
        ...
        if alg == "mess":
            row.update(cost_metrics or _mess_cost_metrics({}, cfg, m, ess_vec))
        else:
            row.update(
                {
                    "mess_cost_metrics_available": False,
